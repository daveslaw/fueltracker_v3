...
import { saveCashierFuelPos, type FuelPosLineInput } from '../actions'
...
export function FuelPosForm({ shiftId, grades, existingLines, existingPhotoUrl }: Props) {
  ...
  async function handlePhotoChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      body.append('file', compressed, `fuel-z-${shiftId}.jpg`)
  ...
  function handleSubmit(e: React.FormEvent) {
    e.preventDefault()
    setError(null)
    const validLines: FuelPosLineInput[] = lines
    ...
    startTransition(async () => {
      const result = await saveCashierFuelPos(shiftId, photoUrl, null, validLines)
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="space-y-3">
        ...
        <p className="text-xs text-gray-400">Leave a grade blank if not sold this shift.</p>
      </div>
